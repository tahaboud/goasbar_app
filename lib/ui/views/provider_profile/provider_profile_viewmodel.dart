...
class ProviderProfileViewModel
    extends FutureViewModel<List<ProviderPublicExperienceResults?>?> {
  int? providerId;
  ...
  Future getProviderPublicExperiencesFromNextPage({int? index}) async {
    if (index! % 10 == 0) {
      pageNumber++;
      ProviderPublicExperienceModel? providerPublicExperienceList =
          await _providerApiService.getProviderPublicExperiences(
              providerId: providerId, page: pageNumber);
      providerPublicExperience!.results!
          .addAll(providerPublicExperienceList!.results!);
      notifyListeners();
    }
  }

  Future<List<ProviderPublicExperienceResults>?>?
      getProviderPublicExperiences() async {
    providerPublicExperience = await _providerApiService
        .getProviderPublicExperiences(providerId: providerId, page: pageNumber);
    notifyListeners();
