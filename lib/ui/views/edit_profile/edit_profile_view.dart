                    ...
                    children: model.isBusy ? [const Loader().center()] : [
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                      ...
                                      image: DecorationImage(
                                        image: model.file == null ? NetworkImage("$baseUrl${model.user!.image}",)
                      ...
                      Container(
                        ...
                        onTap: firstName.text != model.user!.firstName!
                            ...
                            || email.text != model.user!.email!
                            || phone.text != model.user!.phoneNumber!
                            || model.gender.text[0] != model.user!.gender!
                            || model.file != null ? () async {

                          Map<String, dynamic>? body = {};
                          if (firstName.text != model.user!.firstName!) body.addAll({'first_name': firstName.text});
                          if (lastName.text != model.user!.lastName!) body.addAll({'last_name': lastName.text});
                          if (userName.text != model.user!.userName!) body.addAll({'user_name': userName.text});
                          if (email.text != model.user!.email!) body.addAll({'email': email.text});
                          if (phone.text != model.user!.phoneNumber!) body.addAll({'phone_number': phone.text});
                          if (model.gender.text[0] != model.user!.gender!) body.addAll({'gender': model.gender.text[0]});
                          if (model.file != null) {
                          ...
                          MotionToast.warning(
                            title: const Text("Update Your Data First"),
                            description: const Text("To submit update information, you must change your data first"),
